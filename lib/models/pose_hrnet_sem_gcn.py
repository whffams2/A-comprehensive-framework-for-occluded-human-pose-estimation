...
class MMGCN_ALL(nn.Module):
    ...
    def __init__(self, adj, adj_ext1, adj_ext2, adj_ext3, skeleton_graph, hid_dim,output_joints, heatmap_size, non_local=False, coords_dim=(2, 2),
        ...
        self.FC = nn.Sequential(nn.Sequential(nn.Linear((heatmap_size[0] * 2) * (heatmap_size[1] * 2), 1024), nn.ReLU(inplace=True)),
        ...
        self.gconv_layer_last = _ResGraphConv(adj, adj_ext1, adj_ext2, adj_ext3, hid_dim + output_joints, hid_dim + output_joints,
        ...
        self.gconv_output = ExtendSkeletonGraphConv(hid_dim + output_joints, coords_dim[1], adj, adj_ext1, adj_ext2, adj_ext3,
                                                    skeleton_graph=skeleton_graph)
...
class PoseHighResolutionNetSem(nn.Module):

    def __init__(self, cfg, **kwargs):
        self.inplanes = 64
        extra = cfg['MODEL']['EXTRA']
        super(PoseHighResolutionNetSem, self).__init__()
        ...
        self.channel_att = ChannelAttention(32, 32)
        self.spatial_att = Spatial_Attention(32, downsample=True)
...
'''
    ...
    def forward(self, x):
        ...
        context_embeddings = self.context_head(y_list[0])  # 1 * 32  # region context embedding
        ...
        # spatial attention
        satt_features = self.spatial_att(init_features)
        if self.scale_factor > 1:
            ...
            satt_features = F.interpolate(satt_features, scale_factor=self.scale_factor, mode="bilinear",
                                         align_corners=False)
        concat_features = torch.cat([satt_features, catt_features], dim=1)
...
def get_pose_net(cfg, is_train, **kwargs):
    model = PoseHighResolutionNetSem(cfg, **kwargs)
...
def get_gcn_net(cfg, adj, adj_ext_1, adj_ext_2, adj_ext_3, is_train, **kwargs):
    model = MMGCN_ALL(adj, adj_ext_1, adj_ext_2, adj_ext_3, skeleton_graph=cfg['GCN']['SKELETON_GRAPH'],
                      hid_dim=cfg['GCN']['CHANNELS'], non_local=cfg['GCN']['Non_Local'],output_joints=cfg['MODEL']['NUM_JOINTS'],
