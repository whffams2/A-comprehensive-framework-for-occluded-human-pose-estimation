...
class InstanceProvider():
    def __init__(self, INSTANCE_ANN_FILE, INSTANCE_ROOT_DIR, scale_range,
                 part_image_size=[256, 256], num_parts_per_augmentation=[0, 2, 4, 6, 8]):
        AUG_PART = ['All', 'Upper-body-with-head']
        # HEAD_PART = ['Hat', 'Hair', 'Sunglasses', 'face', 'Head']
        # CLOTHES = ['UpperClothes', 'Dress', 'Coat', 'Socks', 'Pants', 'Jumpsuits', 'Scarf', 'Skirt', 'Left-shoe',
        #            'Right-shoe', ]
        self.candidate_parts = AUG_PART
        ...
        part_annot_file = INSTANCE_ANN_FILE
        self.part_root_dir = INSTANCE_ROOT_DIR
    ...
    def generate_part(self, ann, body_bbox, do_SA=True):
        ...
        # self.part_image_size[1]:256 self.part_image_size[0]:192 -> shape:(1 256 192 3)
        part_imgs = np.zeros((max_num_augmentation, self.part_image_size[1], self.part_image_size[0], 3),
                             dtype=np.uint8)
        # shape:(1 256 192 1)
        part_masks = np.zeros((max_num_augmentation, self.part_image_size[1], self.part_image_size[0], 1),
                              dtype=np.float32)
        thetas = np.zeros((max_num_augmentation, 2, 3), dtype=np.float32)
        types = -np.ones(max_num_augmentation, dtype=np.int64)

        joints_vis = np.array(ann['joints_3d_visible'])[:, 0]
        ...
        for i in range(num_augmentation):
            ...
            # path = os.path.join(self.part_root_dir, t_type, part_ann['file_name'])
            ...
            part_img, part_joints = self.adjust_scale_by_diagnal(part_img, part_joints, body_bbox,
                                                                 self.part_scale[t_type], scale_factor)
