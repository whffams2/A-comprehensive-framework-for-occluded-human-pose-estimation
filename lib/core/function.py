...
def gcn_train(config, train_loader, model, gcn, criterion, optimizer,
              gcn_optimizer, epoch, output_dir, tb_log_dir, writer_dict):
    batch_time = AverageMeter()
    data_time = AverageMeter()
    losses = AverageMeter()
    ...
    for i, (input, target, target_weight, all_ins_target, all_ins_target_weight, meta) in enumerate(train_loader):
        ...
        device = input.device
        outputs = model(input)
        # outputs, multi_ouputs = model(input)
        gts = meta['joints'][:, :, :2].to(device)
        # joints_ori = meta['joints_ori'].to(device)
        center = meta['center'].to(device)
        scale = meta['scale'].to(device)
        ...
        # if isinstance(multi_ouputs, list):
        #     loss_multi_heatmap = criterion_heatmap(multi_ouputs[0], target, target_weight)
        #     for outputs in multi_ouputs[1:]:
        #         loss_multi_heatmap += criterion_heatmap(outputs, target, target_weight)
        # else:
        #     output = multi_ouputs
        #     loss_multi_heatmap = criterion_heatmap(output, target, target_weight)
